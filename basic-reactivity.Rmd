...
```{r connection, fig.cap=demo$caption("响应式意味着输出会随着输入的变化而自动更新，就像在这个 app 中我输入“J”、“o”、“e”一样。"), echo = FALSE, out.width = "25%", fig.show="hold", fig.align = "default", message = FALSE}
...
```{r graph-simple, fig.cap = "响应式图显示了输入和输出的连接方式", echo = FALSE, out.width = NULL}
...
```{r graph-collapsed, fig.cap = "响应式图组件使用的形状唤起了它们连接的方式。", echo = FALSE, out.width = NULL}
...
```{r graph-expression, fig.cap = "响应式表达式两边都有角度，因为它将输入连接到输出。", echo = FALSE, out.width = NULL}
...
```{r prod-consumer, echo = FALSE, out.width = NULL, fig.cap = "输入和表达式是响应式生产者；表达式和输出是响应式消费者。"}
...
```{r ttest, echo = FALSE, out.width = "100%", fig.cap=demo$caption("一个 Shiny app，可让您使用 t-test 和频数多边形来比较两个模拟分布。"), message = FALSE}
...
```{r ttest-react1, echo = FALSE, out.width = NULL, fig.cap = "响应式图显示每个输出都取决于每个输入"}
...
```{r ttest-react2, echo = FALSE, out.width = NULL, fig.cap="使用响应式表达式可以大大简化图，使其更容易理解"}
...
```{r ttest-module, echo = FALSE, out.width = NULL, fig.cap="模块强制 app 各部分之间的隔离"}
...
```{r sim, echo = FALSE, out.width = "100%", fig.cap = demo$caption("一个更简单的 app，显示从两个泊松分布中提取的随机数的频率多边形。"), message = FALSE}
...
```{r sim-timer, echo = FALSE, out.width = NULL, fig.cap="`reactiveTimer(500)` 引入了一种新的响应式输入，每半秒自动失效一次"}
...
```{r sim-button1-react, echo = FALSE, out.width = NULL, fig.cap="这个响应式图并没有实现我们的目标；我们添加了一个依赖项，而不是替换现有的依赖项。"}
...
```{r sim-button2-react, echo = FALSE, out.width = NULL, fig.cap="`eventReactive()` 可以将依赖项（黑色箭头）与用于计算结果的值（浅灰色箭头）分开。"}
...
```{r observer, echo = FALSE, out.width = NULL, fig.cap="在响应式图中，observer 看起来与 output 相同"}
